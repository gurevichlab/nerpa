...
impl NRP_Linearizations_Info {
    pub fn load_nrps_linearizations(path: &Path) -> Vec<NRP_Linearizations_Info> {
        let data = std::fs::read_to_string(path)
            .expect("Failed to read NRP linearizations file");
        let nrps: Vec<NRP_Linearizations_Info> = serde_json::from_str(&data)
            .expect("Failed to parse NRP linearizations JSON");
        nrps
    }
}
...
impl HMM {
    pub fn num_states(&self) -> usize { self.transitions.len() }
    pub fn check_shapes(&self) {
        assert_eq!(self.transitions.len(), self.emissions.len(), "states mismatch");
        let mut a = 0usize;
        for e in &self.emissions { if !e.is_empty() { a = e.len(); break; } }
        for e in &self.emissions { if !e.is_empty() { assert_eq!(e.len(), a); } }
    }
    pub fn load_hmms(path: &Path) -> Vec<HMM> {
        ...
        for hmm in &hmms { hmm.check_shapes(); }
        hmms
    }
}

pub fn max_if_0(value: usize) -> usize {
    if value > 0 { value } else { usize::MAX }
}
...
impl MatchInfo <'_> {
    pub fn dump_json_list(matches: &[MatchInfo], path: &Path) {
        let data = serde_json::to_string_pretty(matches)
            .expect("Failed to serialize matches to JSON");
        std::fs::write(path, data)
            .expect("Failed to write matches JSON to file");
    }
}
