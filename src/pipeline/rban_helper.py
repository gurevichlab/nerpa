...
class rBAN_Helper:
    ...
    def run_rban(self,
        ...
        if output_file is None:
            output_file = self.output_cfg.default_output_file
        ...
        command = ['java', '-jar', str(self.config.rban_jar),
                   ...
                   '-outputFileName', str(output_file.name),
                   '-monomersDB', str(self.output_cfg.default_monomers_file)]  # TODO: refactor

        nerpa_utils.sys_call(command, log)

        rban_records = json.load((self.output_cfg.rban_output_dir / output_file).open('r'))
    ...
    def get_hybrid_monomers_per_record(self,
        ...
        self.run_rban(trimmed_monomers,
                      ...
                      output_file=self.output_cfg.putative_hybrids_output_file,
                      log=log)
        ...
        new_monomers_processed = json.load((self.output_cfg.rban_output_dir /
                                            Path(self.output_cfg.putative_hybrids_output_file)).open('r'))
