...
use super::{
    common_types::MonomerIdx,
    monomer_graph::{AtomData, Monomer, MonomerFeatures, MonomerGraph},
    parsed_rban_record::{AtomId, MonomerInfo, Parsed_rBAN_Record},
};
...
impl Monomer {
    pub fn from_rban_record(rban_record: &Parsed_rBAN_Record, idx: MonomerIdx) -> Self {
        let monomer_info = rban_record
            .monomers
            .get(&idx)
        ...
        let atoms_with_data = monomer_info
            ...
            .collect();
        let atoms_set: HashSet<AtomId> = monomer_info.atoms.iter().cloned().collect();
        let atomic_bonds = rban_record
            .atomic_bonds
            .iter()
