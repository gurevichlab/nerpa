...
fn relax_non_emitting_states(dp: &mut DP_Table, hmm: &HMM, v: usize, w: usize) {
    for _pass in 0..2 {
	    ...
	    let cur_coords = DP_Coords{vertex: v, weight: w, state: s};
		...
		let new_coords = DP_Coords{vertex: v, weight: w, state: to};
		dp.update(&new_coords,
			  &cur_coords,
			  Some(edge_lp),
			  None);
            }
...
fn advance_dag_unlabeled<'a>(
	...
	let cur_coords = DP_Coords{vertex: v, weight: w, state: s};
		...
		let new_coords = DP_Coords{vertex: dag_edge.to, weight: new_weight, state: s};
		dp.update(&new_coords,
			  &cur_coords,
			  None,
			  Some(dag_edge.clone()));
...
fn advance_dag_labeled<'a>(
	...
	let cur_coords = DP_Coords{vertex: v, weight: w, state: s};
        ...
        for &(new_state, edge_lp) in &hmm.transitions[s] {
		    ...
		    let new_coords = DP_Coords{vertex: dag_edge.to, weight: new_weight, state: new_state};
		    dp.update(&new_coords,
			      &cur_coords,
